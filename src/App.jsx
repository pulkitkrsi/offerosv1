...
function runSimulation(offer, txns) { const sorted = [...txns].sort((a, b) => new Date(a.date) - new Date(b.date)); const cs = sorted[0]?.date ? new Date(sorted[0].date) : null; let sc = 0, tn = 0, tg = 0, qt = 0, wr = parseFloat(offer.w) || 0, ftu = false, fsu = false, totalUsed = 0; const isW = offer.activity === "Wallet top-up", snV = parseInt(offer.sn) || 0, wpcLimit = parseFloat(offer.wpc) || Infinity, wpunMin = parseFloat(offer.wpun) || 0; const rows = txns.map((tx, i) => { const td = tx.date ? new Date(tx.date) : null; const ed = cs ? new Date(cs.getTime() + (parseInt(offer.t) || 30) * 86400000) : null; let rw = 0, st = "", rs = "—", ss = "—", netVal = 0;
  if (isW) { const amt = parseFloat(tx.amount) || 0; tg += amt;
    if (offer.wpre) { if (!td) st = "no-date"; else if (ed && td > ed) st = "expired"; else if (wr <= 0) st = "exhausted"; else if (offer.sx && sc >= parseInt(offer.sx)) st = "max-sess"; else if (totalUsed >= wpcLimit) st = "credit-cap"; else { const u = parseFloat(tx.units || tx.amount) || 0; if (wpunMin > 0 && u < wpunMin && tx.units !== undefined) { st = "below-kwh"; } else { sc++; ss = sc; let ms = wr; if (offer.dist === "spread" && offer.wsx) { const cap = offer.wsxType === "pct" ? (parseFloat(offer.wsx) / 100 * (parseFloat(offer.w) || 0)) : parseFloat(offer.wsx); ms = Math.min(wr, cap); } const remainingCap = wpcLimit - totalUsed; ms = Math.min(ms, remainingCap); rw = Math.min(ms, amt); wr = Math.max(0, wr - rw); totalUsed += rw; rs = "₹" + rw.toFixed(2) + " used"; st = wr <= 0 ? "exhausted-now" : totalUsed >= wpcLimit ? "credit-cap" : "applied"; qt++; } } return { idx: i + 1, date: tx.date || "—", amount: amt, reward: rw, rateStr: rs, sessStr: ss, status: st, type: "preload", netVal: 0 };
...
:root,.light{--bg:#faf8f5;--bg2:#f5f2ed;--bg3:#edeae5;--bg4:#e5e2dd;--border:rgba(60,50,40,.08);--border2:rgba(60,50,40,.14);--text:#1e1a16;--text2:#5c554d;--text3:#8a847c;--accent:#eb212e;--accent2:#c91a25;--accent-bg:rgba(235,33,46,.06);--green:#1a7a3a;--green-bg:rgba(26,122,58,.08);--red:#c62828;--red-bg:rgba(198,40,40,.06);--amber:#b07714;--amber-bg:rgba(176,119,20,.08);--blue:#1565c0;--blue-bg:rgba(21,101,192,.07);--purple:#6a3fa0;--purple-bg:rgba(106,63,160,.06);--teal:#0b796a;--teal-bg:rgba(11,121,106,.06);--sage:#5a8a6a;--sage-bg:rgba(90,138,106,.08);--r:10px;--r2:14px;--r3:18px;--sh-sm:0 1px 2px rgba(40,30,20,.05);--sh:0 1px 2px rgba(40,30,20,.04),0 6px 20px -8px rgba(40,30,20,.1);--sh-lg:0 2px 4px rgba(40,30,20,.05),0 16px 36px -10px rgba(40,30,20,.14);--font-display:'Instrument Serif','Times New Roman',serif;--font-ui:'Inter',-apple-system,sans-serif;--font-mono:'JetBrains Mono',monospace}
.dark{--bg:#121110;--bg2:#1a1918;--bg3:#222120;--bg4:#2c2b28;--border:rgba(255,248,240,.07);--border2:rgba(255,248,240,.12);--text:#f0ece6;--text2:#a8a4a0;--text3:#686460;--accent:#eb212e;--accent2:#ff3a48;--accent-bg:rgba(235,33,46,.1);--green:#4eca6a;--green-bg:rgba(78,202,106,.1);--red:#f05060;--red-bg:rgba(240,80,96,.1);--amber:#e0a830;--amber-bg:rgba(224,168,48,.1);--blue:#60b0e8;--blue-bg:rgba(96,176,232,.1);--purple:#a080d0;--purple-bg:rgba(160,128,208,.08);--teal:#50c8b0;--teal-bg:rgba(80,200,176,.08);--sage:#70b880;--sage-bg:rgba(112,184,128,.1);--sh-sm:0 1px 2px rgba(0,0,0,.2);--sh:0 2px 8px rgba(0,0,0,.3);--sh-lg:0 8px 30px rgba(0,0,0,.4)}
...
/* Sidebar */
.sidebar{background:var(--bg2);border-right:1px solid var(--border);padding:20px 14px;display:flex;flex-direction:column;gap:2px;position:sticky;top:0;height:100vh;overflow-y:auto}
.sb-brand{display:flex;align-items:center;gap:10px;padding:4px 8px 20px;font-family:var(--font-display);font-size:22px;letter-spacing:-0.01em;cursor:pointer}.sb-brand .dot{width:22px;height:22px;border-radius:50%;background:var(--accent);position:relative;flex-shrink:0}.sb-brand .dot::after{content:'';position:absolute;inset:6px;background:var(--bg2);border-radius:50%}
.sb-section{padding:16px 8px 6px;font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:var(--text3)}
.sb-item{display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:8px;color:var(--text2);font-size:13.5px;cursor:pointer;width:100%;text-align:left;border:1px solid transparent;transition:all .12s}.sb-item:hover{background:var(--bg3);color:var(--text)}.sb-item.active{background:var(--bg);color:var(--text);box-shadow:var(--sh-sm);border-color:var(--border)}
.sb-foot{margin-top:auto;padding:14px 8px;border-top:1px solid var(--border);display:flex;align-items:center;gap:10px}
.sb-avatar{width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--sage));color:#fff;display:grid;place-items:center;font-size:12px;font-weight:600;flex-shrink:0}
/* Topbar */
.topbar{display:flex;align-items:center;gap:16px;padding:14px 28px;border-bottom:1px solid var(--border);background:var(--bg);position:sticky;top:0;z-index:10}
.breadcrumb{font-size:13px;color:var(--text3);display:flex;align-items:center;gap:6px}.breadcrumb .sep{opacity:.4}.breadcrumb .current{color:var(--text)}
...
/* Page headers */
.page-hdr{padding:28px 0 20px}.page-hdr-sub{font-size:13px;color:var(--text3);cursor:pointer;display:flex;align-items:center;gap:4px;margin-bottom:6px}.page-hdr-sub:hover{color:var(--text)}
.page-title{font-family:var(--font-display);font-size:42px;line-height:1.05;letter-spacing:-0.015em;font-weight:400}
.page-title em{font-style:italic;color:var(--accent)}
/* Cards */
.card{background:var(--bg);border:1px solid var(--border);border-radius:var(--r3);padding:22px;margin-bottom:16px;box-shadow:var(--sh-sm)}.card-title{font-family:var(--font-display);font-size:22px;margin-bottom:16px;font-weight:400;letter-spacing:-0.01em}
/* Segments */
.seg-cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;margin-bottom:16px}.seg-card{border:1px solid var(--border);border-radius:var(--r2);padding:16px;cursor:pointer;transition:all .15s;background:var(--bg)}.seg-card:hover{border-color:var(--border2);box-shadow:var(--sh-sm)}.seg-card.selected{border-color:var(--accent);background:var(--accent-bg);box-shadow:0 0 0 3px rgba(235,33,46,.08)}.seg-card-icon{font-size:22px;margin-bottom:8px}.seg-card-name{font-weight:600;font-size:14px;margin-bottom:3px}.seg-card-desc{font-size:12px;color:var(--text3);margin-bottom:8px}.seg-card-insight{font-size:11px;color:var(--text2);line-height:1.5;padding-top:8px;border-top:1px solid var(--border);font-style:italic}
/* CSV zone */
.csv-zone{margin-top:16px;padding:18px;border:1px dashed var(--border2);border-radius:var(--r2);background:var(--bg2);transition:all .15s;cursor:pointer;text-align:center}.csv-zone:hover{border-color:var(--teal);background:var(--teal-bg)}.csv-zone.has{border-color:var(--teal);border-style:solid;background:var(--teal-bg)}
.csv-logic{margin-top:12px;display:flex;gap:8px;align-items:center;font-size:12px;color:var(--text3)}
/* Fields */
.field{display:flex;flex-direction:column;gap:5px}.field-label{font-size:12px;color:var(--text2);font-weight:500;display:flex;align-items:center;gap:6px}
.grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
input[type=text],input[type=number],input[type=date],input[type=password],select,textarea{background:var(--bg);border:1px solid var(--border2);border-radius:8px;padding:9px 12px;color:var(--text);font-family:var(--font-ui);font-size:13px;width:100%;transition:border-color .15s}input:focus,select:focus,textarea:focus{outline:none;border-color:var(--accent);box-shadow:0 0 0 3px rgba(235,33,46,.06)}input::placeholder{color:var(--text3)}select{cursor:pointer}select option{background:var(--bg)}
/* Toggles */
.preload-box{margin-top:16px;padding:18px;background:var(--bg2);border:1px solid var(--border);border-radius:var(--r2)}.toggle-row{display:flex;align-items:center;justify-content:space-between}.toggle-label{font-size:13px;font-weight:600}.toggle-sub{font-size:11px;color:var(--text3)}.toggle-track{width:42px;height:24px;border-radius:12px;background:var(--bg4);cursor:pointer;position:relative;transition:.2s;flex-shrink:0}.toggle-track.on{background:var(--sage)}.toggle-thumb{position:absolute;width:18px;height:18px;border-radius:50%;top:3px;left:3px;background:#fff;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.12)}.toggle-track.on .toggle-thumb{transform:translateX(18px)}
/* Distribution */
.dist-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:14px 0}.dist-item{border:1px solid var(--border);border-radius:var(--r2);padding:14px;cursor:pointer;transition:all .15s;background:var(--bg)}.dist-item:hover{border-color:var(--border2)}.dist-item.selected{border-color:var(--sage);background:var(--sage-bg)}.dist-item-title{font-size:12px;font-weight:600;margin-bottom:3px}.dist-item-desc{font-size:11px;color:var(--text3)}
/* Activity */
.act-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px}.act-item{border:1px solid var(--border);border-radius:var(--r3);padding:18px;cursor:pointer;transition:all .15s;background:var(--bg)}.act-item:hover{border-color:var(--border2);box-shadow:var(--sh-sm)}.act-item.selected{border-color:var(--accent);background:var(--accent-bg);box-shadow:0 0 0 3px rgba(235,33,46,.08)}.act-item-icon{font-size:24px;margin-bottom:8px}.act-item-title{font-size:14px;font-weight:600;margin-bottom:3px}.act-item-vars{font-size:12px;color:var(--text3)}
/* Reward */
.rwd-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px}.rwd-item{border:1px solid var(--border);border-radius:var(--r2);padding:16px;cursor:pointer;transition:all .15s;background:var(--bg)}.rwd-item:hover{box-shadow:var(--sh-sm)}.rwd-item.selected{border-color:var(--accent);background:var(--accent-bg)}.rwd-item.disabled{opacity:.25;pointer-events:none}.rwd-item-name{font-size:14px;font-weight:600;margin-bottom:3px}.rwd-item-formula{font-size:12px;color:var(--text3)}
.rwd-disabled{padding:14px 16px;background:var(--sage-bg);border:1px solid rgba(90,138,106,.15);border-radius:var(--r);font-size:13px;color:var(--sage);margin-bottom:18px;line-height:1.6}
.rwd-config{background:var(--bg2);border:1px solid var(--border);border-radius:var(--r2);padding:18px 22px;margin-top:8px}.rwd-config-title{font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;color:var(--text3);margin-bottom:16px}
.tier-hdr,.tier-row{display:grid;grid-template-columns:80px 1fr 28px;gap:10px;margin-bottom:8px;align-items:center}.tier-hdr span{font-size:11px;color:var(--text3);font-weight:600;text-transform:uppercase;letter-spacing:.04em}
/* Wallet tiers */
.wt-box{margin-top:18px;padding:18px 20px;border:1px solid rgba(235,33,46,.1);border-radius:var(--r2);background:var(--accent-bg)}.wt-title{font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:14px}
.wt-modes{display:flex;gap:8px;margin-bottom:14px}.wt-mode{padding:7px 16px;border-radius:22px;border:1px solid var(--border2);background:var(--bg);color:var(--text2);font-size:12px;cursor:pointer;font-weight:500;transition:all .15s}.wt-mode:hover{border-color:var(--text3)}.wt-mode.active{border-color:var(--accent);color:var(--accent);background:var(--accent-bg)}
.slab-hdr,.slab-row{display:grid;grid-template-columns:80px 80px 70px 28px;gap:8px;margin-bottom:6px;align-items:center}.slab-hdr span{font-size:11px;color:var(--text3);font-weight:600;text-transform:uppercase;letter-spacing:.04em}
/* Boundaries */
...
/* Summary */
.sum-card{border:1px solid var(--border);border-radius:var(--r3);overflow:hidden;margin-bottom:18px;box-shadow:var(--sh-sm)}.sum-hdr{padding:16px 22px;background:var(--bg2);border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center}.sum-name{font-family:var(--font-display);font-size:20px}
...
.sum-body{padding:22px}.sum-plain{font-size:13px;line-height:1.9;color:var(--text2);margin-bottom:22px;padding:16px 18px;background:var(--bg2);border-radius:var(--r);border-left:3px solid var(--accent)}.sum-plain strong{color:var(--accent);font-weight:600}
.vtable{width:100%;border-collapse:collapse}.vtable th{font-size:11px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:var(--text3);text-align:left;padding:8px 12px;border-bottom:1px solid var(--border)}.vtable td{padding:8px 12px;border-bottom:1px solid var(--border);font-size:13px}.vtable tr:last-child td{border-bottom:none}.vtable tr:hover td{background:var(--bg2)}
/* Simulation */
.txn-hdr,.txn-row{display:grid;grid-template-columns:130px 90px 80px 28px;gap:10px;margin-bottom:8px;align-items:center}.txn-hdr span{font-size:11px;color:var(--text3);font-weight:600;text-transform:uppercase;letter-spacing:.04em}
.sample-btn{width:100%;padding:12px;margin-bottom:16px;border:1px dashed var(--accent);border-radius:var(--r);background:var(--accent-bg);color:var(--accent);font-size:13px;font-weight:500;cursor:pointer;transition:all .15s}.sample-btn:hover{background:rgba(235,33,46,.1)}
/* Dashboard metrics */
.dash-section{margin-bottom:28px}.dash-title{font-family:var(--font-display);font-size:22px;margin-bottom:16px;display:flex;align-items:center;gap:12px;font-weight:400}.dash-icon{width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0}.di-g{background:var(--green-bg);color:var(--green)}.di-a{background:var(--amber-bg);color:var(--amber)}.di-r{background:var(--accent-bg);color:var(--accent)}
.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px}.mc{background:var(--bg);border:1px solid var(--border);border-radius:var(--r3);padding:18px;box-shadow:var(--sh-sm)}.mc-label{font-size:11px;font-weight:500;color:var(--text3);letter-spacing:.04em;text-transform:uppercase;margin-bottom:8px}.mc-val{font-family:var(--font-display);font-size:28px;letter-spacing:-0.01em}.mc-sub{font-size:11px;color:var(--text3);margin-top:5px}
/* Result table */
.result-tbl{width:100%;border-collapse:collapse;font-size:12px}.result-tbl th{font-size:11px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:var(--text3);text-align:left;padding:10px 10px;border-bottom:1px solid var(--border)}.result-tbl td{padding:10px;border-bottom:1px solid var(--border);color:var(--text2)}.result-tbl tr:last-child td{border-bottom:none}.result-tbl tr:hover td{background:var(--bg2)}.result-tbl td.rval{color:var(--accent);font-family:var(--font-mono);font-weight:600}
/* Badges */
.sbadge{font-size:11px;padding:3px 10px;border-radius:22px;font-weight:500;display:inline-block}.bg-green{background:var(--green-bg);color:var(--green)}.bg-amber{background:var(--amber-bg);color:var(--amber)}.bg-red{background:var(--red-bg);color:var(--red)}.bg-blue{background:var(--blue-bg);color:var(--blue)}.bg-muted{background:var(--bg3);color:var(--text3)}
/* Risk flags */
...
/* Buttons */
.btn-row{display:flex;gap:10px;margin-top:22px}.btn{padding:10px 18px;border-radius:8px;border:1px solid var(--border2);background:var(--bg);color:var(--text2);font-family:var(--font-ui);font-size:13px;font-weight:500;cursor:pointer;transition:all .12s;display:inline-flex;align-items:center;gap:8px}.btn:hover{background:var(--bg2);color:var(--text)}.btn-primary{background:var(--accent);border-color:var(--accent);color:#fff}.btn-primary:hover{background:var(--accent2)}.btn-dashed{width:100%;padding:10px;border:1px dashed var(--border2);border-radius:8px;background:none;font-size:12px;color:var(--text3);cursor:pointer;font-weight:500;margin-top:8px;transition:all .15s}.btn-dashed:hover{border-color:var(--accent);color:var(--accent)}.del-btn{background:none;border:none;color:var(--text3);cursor:pointer;font-size:15px;transition:color .15s}.del-btn:hover{color:var(--red)}.scroll-x{overflow-x:auto}
/* Steps */
.steps{display:flex;gap:0;margin-bottom:20px;overflow-x:auto;background:var(--bg2);border:1px solid var(--border);border-radius:var(--r2);padding:4px}.step{flex:1;padding:10px 8px;font-size:12px;cursor:pointer;color:var(--text3);text-align:center;border-radius:8px;transition:all .15s;white-space:nowrap;font-weight:500}.step:hover{color:var(--text2)}.step.active{background:var(--bg);color:var(--text);box-shadow:var(--sh-sm)}.step.done{color:var(--green)}.step.incomplete{color:var(--red)}
/* Offers grid */
.offers-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:14px}.offer-card{background:var(--bg);border:1px solid var(--border);border-radius:var(--r3);padding:18px;cursor:pointer;transition:all .15s;position:relative}.offer-card:hover{box-shadow:var(--sh);border-color:var(--border2)}.offer-card.active{border-color:var(--accent)}.offer-card-actions{position:absolute;top:12px;right:12px;display:flex;gap:4px;opacity:0;transition:opacity .15s}.offer-card:hover .offer-card-actions{opacity:1}.offer-card-actions button{width:28px;height:28px;border-radius:6px;border:1px solid var(--border);background:var(--bg);font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text3);transition:all .12s}.offer-card-actions button:hover{background:var(--bg3);color:var(--text)}.offer-card-actions button.del:hover{color:var(--red);border-color:var(--red)}
.offer-card-type{display:flex;align-items:center;gap:8px;margin-bottom:10px}.offer-card-dot{width:8px;height:8px;border-radius:50%}.offer-card-label{font-size:11px;color:var(--text3);font-weight:500;text-transform:uppercase;letter-spacing:.04em}.offer-card-name{font-family:var(--font-display);font-size:18px;margin-bottom:4px;letter-spacing:-0.01em}.offer-card-segs{font-size:12px;color:var(--text3)}
...
/* Campaign cards */
.campaign-list{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:14px;margin-bottom:22px}
.campaign-card{background:var(--bg);border:1px solid var(--border);border-radius:var(--r3);padding:22px;cursor:pointer;transition:all .15s;position:relative}.campaign-card:hover{box-shadow:var(--sh);border-color:var(--border2)}.campaign-card-name{font-family:var(--font-display);font-size:20px;margin-bottom:8px}.campaign-card-meta{font-size:12px;color:var(--text3);display:flex;gap:14px}.campaign-card-status{font-size:11px;padding:4px 10px;border-radius:22px;font-weight:600;position:absolute;top:16px;right:16px}
/* Save indicator */
...
/* Theme toggle */
.theme-toggle{width:34px;height:20px;border-radius:10px;background:var(--bg4);cursor:pointer;position:relative;transition:.2s;border:1px solid var(--border2);flex-shrink:0}.theme-toggle .thumb{position:absolute;width:14px;height:14px;border-radius:50%;top:2px;left:2px;background:var(--text3);transition:.2s}.dark .theme-toggle .thumb{transform:translateX(14px);background:var(--accent)}
/* User badge */
.user-badge{display:flex;align-items:center;gap:8px;padding:4px;cursor:pointer;border-radius:8px;transition:background .15s}.user-badge:hover{background:var(--bg3)}.user-badge-avatar{width:28px;height:28px;border-radius:50%;background:var(--accent);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600}.user-badge-name{font-size:12px;font-weight:500;color:var(--text2)}
/* Modals */
.modal-overlay{position:fixed;inset:0;background:rgba(30,26,22,.3);display:flex;align-items:center;justify-content:center;z-index:200;backdrop-filter:blur(6px)}.modal{background:var(--bg);border:1px solid var(--border);border-radius:var(--r3);padding:30px;max-width:460px;width:92%;box-shadow:var(--sh-lg)}.modal-title{font-family:var(--font-display);font-size:22px;margin-bottom:8px;font-weight:400}.modal-msg{font-size:13px;color:var(--text2);margin-bottom:22px;line-height:1.6}.modal-actions{display:flex;gap:10px;justify-content:flex-end}
/* Login */
.login-page{min-height:100vh;display:flex;align-items:center;justify-content:center;background:var(--bg2);padding:20px}
.login-card{background:var(--bg);border:1px solid var(--border);border-radius:var(--r3);padding:44px;max-width:400px;width:100%;box-shadow:var(--sh-lg)}
.login-logo{width:50px;height:50px;border-radius:14px;background:var(--accent);display:flex;align-items:center;justify-content:center;color:#fff;font-family:var(--font-display);font-size:26px;margin:0 auto 22px;box-shadow:0 4px 12px rgba(235,33,46,.25)}
.login-title{font-family:var(--font-display);font-size:26px;text-align:center;margin-bottom:4px;font-weight:400}.login-sub{font-size:13px;color:var(--text3);text-align:center;margin-bottom:30px}
.login-field{margin-bottom:18px}.login-field label{display:block;font-size:12px;font-weight:600;color:var(--text2);margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em}
.login-err{padding:12px 16px;background:var(--red-bg);border:1px solid rgba(198,40,40,.12);border-radius:8px;font-size:13px;color:var(--red);margin-bottom:18px;text-align:center}
.login-btn{width:100%;padding:14px;background:var(--accent);border:none;border-radius:8px;color:#fff;font-family:var(--font-ui);font-size:14px;font-weight:600;cursor:pointer;transition:background .15s;margin-top:8px}.login-btn:hover{background:var(--accent2)}.login-btn:disabled{opacity:.5;cursor:not-allowed}
/* AI drawer */
.ai-overlay{position:fixed;inset:0;background:rgba(30,26,22,.3);z-index:30;opacity:0;pointer-events:none;transition:opacity .2s;backdrop-filter:blur(4px)}.ai-overlay.open{opacity:1;pointer-events:auto}
...
.ai-x{background:none;border:none;color:var(--text3);cursor:pointer;font-size:18px;padding:4px 8px;border-radius:6px;transition:all .12s}.ai-x:hover{background:var(--bg3);color:var(--text)}
.ai-body{flex:1;overflow-y:auto;padding:18px}
.ai-msg{margin-bottom:14px;padding:14px 16px;border-radius:var(--r2);font-size:13px;line-height:1.7;color:var(--text2)}.ai-msg.bot{background:var(--bg2);border:1px solid var(--border)}.ai-msg.user{background:var(--text);color:var(--bg);border-radius:var(--r2) var(--r2) 4px var(--r2)}
.ai-msg strong{color:var(--accent);font-weight:600}.ai-msg code{font-family:var(--font-mono);font-size:11px;background:var(--bg3);padding:2px 5px;border-radius:4px}
...
.ai-ftr{padding:16px;border-top:1px solid var(--border)}.ai-ftr-row{display:flex;gap:8px}.ai-inp{flex:1;background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:10px 12px;color:var(--text);font-size:12px}.ai-inp:focus{outline:none;border-color:var(--accent)}.ai-go{background:var(--accent);border:none;border-radius:8px;color:#fff;font-size:12px;padding:8px 16px;cursor:pointer;font-weight:500}.ai-go:hover{background:var(--accent2)}
.ai-qa{display:flex;gap:6px;margin-top:10px;flex-wrap:wrap}.ai-qa button{padding:5px 12px;font-size:11px;border:1px solid var(--border);border-radius:22px;background:var(--bg);color:var(--text3);cursor:pointer;font-weight:500;transition:all .12s}.ai-qa button:hover{border-color:var(--accent);color:var(--accent)}
/* Responsive */
...
export default function App(){
  ...
  const[offers,setOffers]=useState([]);const[allOffers,setAllOffers]=useState([]);const[cid,setCid]=useState(null);
  ...
  useEffect(()=>{if(activeCampaign){api("/offers?campaignId="+activeCampaign._id).then(o=>{setOffers(o);setCid(null);setView("offers");setAllOffers(prev=>{const otherOffers=prev.filter(p=>p.campaignId!==activeCampaign._id);return[...otherOffers,...o]})}).catch(()=>{});setMarginPct(activeCampaign.marginPct||30)}},[activeCampaign]);
  ...
  return<><style>{css}</style><div className="app">
    <aside className="sidebar">
      <div className="sb-brand" onClick={()=>{setActiveCampaign(null);setView("campaigns")}}>
        <span className="dot"/><span>Offer<span style={{fontStyle:"italic",color:"var(--accent)"}}>OS</span></span>
      </div>
      ...
      <div className="sb-foot">
        <div className="sb-avatar">{user.displayName?.[0]?.toUpperCase()||"U"}</div>
        <div style={{lineHeight:1.2,flex:1}}>
          <div style={{fontSize:13,fontWeight:500}}>{user.displayName||"User"}</div>
          <div style={{fontSize:11,color:"var(--text3)"}}>{user.role||"editor"}</div>
        </div>
